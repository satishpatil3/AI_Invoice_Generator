...
const AllInvoices = () => {
  ...
  const handleStatusChange = async (invoice) => {
    ...
    try {
      ...
      setInvoices((prev) =>prev.map((inv) =>inv._id === invoice._id ? response.data : inv));
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Mange your all invoices in one place
          </p>
        </div>
        ...
        <div className="p-4 sm:p-6 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-shrink-0">
              <select
                className="w-full sm:w-auto h-10 pl-2 pr-2 py-2 border border-slate-200 rounded-lg bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={statusFilter}
        ...
        {filteredInvoices.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-slate-500 mb-6 max-w-md">
              Your search or filter criteria didn’t match any invoices.Try adjusting Your Search.
            </p>
            {invoices.length === 0 && (
          ...
          <div className="w-[90vw] md:w-auto overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wide">Invoice #</th>
